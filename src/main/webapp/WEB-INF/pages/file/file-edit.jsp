...
<form method="POST" action="<spring:url value="/category-edit-handler" />">
    ...
    <div class="form-group <c:if test="${errors.get(\"chpu\") != null}">has-error</c:if>">
        <label for="chpu">ЧПУ</label>
        <input type="text" class="form-control" name="chpu" id="chpu" placeholder="ЧПУ" value="${category.chpu}">
        <c:if test="${errors.get(\"chpu\") != null}">
            <span class="help-block">
                <c:forEach items="${errors.get(\"chpu\")}" var="item" varStatus="itemStat">
                    ${item}
                </c:forEach>
            </span>
        </c:if>
